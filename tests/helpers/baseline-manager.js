...
class BaselineManager {
	constructor(options = {}) {
		...
		this.localCacheDir =
			options.localCacheDir || path.join(__dirname, "../.baselines");
		...
		const hasAWSCredentials =
			!!process.env.AWS_ACCESS_KEY_ID || !!process.env.HOME;
	...
	async checkAwsSsoStatus() {
		...
		if (this.isCI) {
			return {
				loggedIn: true,
				profile: "CI",
				message: "Running in GitHub Actions (using IAM credentials)",
			};
		}
		...
		try {
			...
			const result = execSync(
				`aws sts get-caller-identity --profile ${this.profile} 2>&1`,
				{
					encoding: "utf-8",
					timeout: 10000,
				},
	...
	async promptSsoLogin() {
		...
		console.log(
			"  ║  To upload baselines to S3, please run:                    ║",
		);
	...
	async minifyPng(imageBuffer) {
		try {
			...
			console.log(
				`  PNG minification: ${originalSize} → ${minifiedSize} bytes (${savings}% reduction)`,
			);
	...
	async uploadBaseline(testName, browser, screenshotBuffer) {
		...
		try {
			...
			if (
				...
				await this.promptSsoLogin();
				console.log(
					`  ⚠️  Baseline NOT uploaded to S3 - AWS SSO session expired`,
				);
	...
	async downloadBaseline(testName, browser) {
		...
		if (!this.awsConnectionVerified && !this.isCI) {
			...
			if (!ssoStatus.loggedIn) {
				console.log(
					`  ⚠️  Cannot fetch baseline from S3 - ${ssoStatus.message}`,
				);
				if (ssoStatus.hint) {
	...
	async compareImages(actual, baseline, threshold = 0.05) {
		try {
			...
			if (
				...
				console.warn(
					`  Dimension mismatch: actual ${actualMeta.width}x${actualMeta.height} vs baseline ${baselineMeta.width}x${baselineMeta.height}`,
				);
				return {
					match: false,
			...
			console.log(
				`  Image comparison: ${diffPixels} pixels differ (${diffPercent.toFixed(2)}%), threshold: ${threshold * 100}%`,
			);
	...
	async clearCache() {
		try {
			...
			await Promise.all(
				files.map((f) => fs.unlink(path.join(this.localCacheDir, f))),
			);
