...
test.describe("WebGL Performance Budgets", () => {
	...
	test("orbit scene initialization should complete within 150ms @performance @init", async ({
		page,
		browserName,
	}) => {
		console.log(
		...
		const initMetrics = await page.evaluate(() => {
			...
			const orbitInit = measures.find((m) =>
				m.name.includes("OrbitScene-init"),
			);
		...
		console.log(
			`  DOM Content Loaded: ${initMetrics.domContentLoaded?.toFixed(2)}ms`,
		);

		if (initMetrics.orbitInitDuration !== null) {
			console.log(
				`  OrbitScene initialization: ${initMetrics.orbitInitDuration.toFixed(2)}ms`,
			);
			expect(initMetrics.orbitInitDuration).toBeLessThan(150);
		} else {
			console.log(
				`  ⚠️  OrbitScene performance marks not found - checking manual timing`,
			);

			// Fallback: Measure time from page load to orbit visibility
			const fallbackTiming = await page.evaluate(() => {
				const _start = performance.timing.navigationStart;
				const now = performance.now();
				return now;
			});

			console.log(
				`  Fallback timing (page load to orbit ready): ${fallbackTiming.toFixed(2)}ms`,
			);
	...
	test.skip(
		!!process.env.CI,
		"FPS measurement requires GPU hardware not available in CI",
	);
	test("should maintain >50 FPS during orbit animation @performance @fps", async ({
		page,
		browserName,
	}) => {
	...
	test("should use <200MB WebGL memory after 30s runtime @performance @memory", async ({
		page,
		browserName,
	}) => {
		...
		test.skip(
			!!process.env.CI,
			"Memory measurement requires GPU hardware not available in CI",
		);
		...
		if (!memoryData.hasMemoryAPI) {
			console.log(
				`  ⚠️  performance.memory API not available (only works in Chrome with --enable-precise-memory-info)`,
			);
		...
		console.log(
			`  Memory growth: ${(finalSample.usedMB - initialSample.usedMB).toFixed(1)}MB`,
		);

		// Check for memory leaks (growth >50MB over 30s indicates leak)
		const memoryGrowth =
			parseFloat(finalSample.usedMB) - parseFloat(initialSample.usedMB);
	...
	test("should handle rapid scene visibility changes without performance degradation @performance @stress", async ({
		page,
		browserName,
	}) => {
		...
		console.log(
			`  Average hide time: ${stressResults.avgHideTime.toFixed(2)}ms`,
		);
		console.log(
			`  Average show time: ${stressResults.avgShowTime.toFixed(2)}ms`,
		);
	...
	test("should gracefully handle reduced motion preference @performance @accessibility", async ({
		page,
	}) => {
		...
		console.log(
			`  Prefers reduced motion: ${reducedMotionState.prefersReducedMotion}`,
		);
		console.log(
			`  SceneInitializer exists: ${reducedMotionState.sceneInitializerExists}`,
		);
