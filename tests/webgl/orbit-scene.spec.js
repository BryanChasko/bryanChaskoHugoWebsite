...
test.describe("Builder Card Orbit Scene", () => {
	...
	test.skip(
		({ browserName }) => browserName === "firefox" && !!process.env.CI,
	...
	test.beforeEach(async ({ page }) => {
		...
		await page.waitForSelector("[data-orbit-scene]", {
			state: "visible",
			timeout: 10000,
		});
	...
	test("should render orbit with teal/green Vibrant Cosmic colors @visual", async ({
		page,
		browserName,
	}) => {
		...
		if (baselineManager.updateBaselines) {
			...
			const baseline = await baselineManager.downloadBaseline(
				testName,
				browserName,
			);
			...
			if (!baseline) {
				...
				test.skip(
					true,
					"Initial baseline created - run tests again to validate",
				);
				return;
			}

			// Compare images with 5% tolerance (antialiasing/lighting variance)
			const comparison = await baselineManager.compareImages(
				screenshot,
				baseline,
				0.05,
			);
			...
			if (!comparison.match) {
				...
				const actualPath = path.join(
					__dirname,
					`../test-results/${testName}-${browserName}-actual.png`,
				);
				await require("node:fs").promises.writeFile(actualPath, screenshot);
				console.log(
					`  ✗ Visual regression detected: ${comparison.diffPercent.toFixed(2)}% pixels differ`,
				);
				console.log(`  Actual screenshot saved: ${actualPath}`);
			} else {
				console.log(
					`  ✓ Visual match: ${comparison.diffPercent.toFixed(2)}% difference (within 5% threshold)`,
				);
			}
	...
	test("should extract canvas pixels and verify color palette @pixel-validation", async ({
		page,
		browserName,
	}) => {
		...
		const colorData = await page.evaluate(async () => {
			...
			console.log(
				`[Test] Canvas info: ${width}x${height}, context source: ${contextSource}`,
			);
			...
			const _scissor = gl.getParameter(gl.SCISSOR_BOX);
			const scissorEnabled = gl.isEnabled(gl.SCISSOR_TEST);
			const blendEnabled = gl.isEnabled(gl.BLEND);
			console.log(
				`[Test] GL state: clearColor=[${clearColor}], viewport=[${viewport}], scissor=${scissorEnabled}, blend=${blendEnabled}`,
			);
			...
			console.log(
				`[Test] First pixel (0,0): [${pixels[0]}, ${pixels[1]}, ${pixels[2]}, ${pixels[3]}]`,
			);
			const centerIdx =
				(Math.floor(height / 2) * width + Math.floor(width / 2)) * 4;
			console.log(
				`[Test] Center pixel: [${pixels[centerIdx]}, ${pixels[centerIdx + 1]}, ${pixels[centerIdx + 2]}, ${pixels[centerIdx + 3]}]`,
			);
		...
		console.log(
			`  Purple fallback pixels detected: ${purplePixels.length} / ${colorData.samples.length}`,
		);
		...
		const vibrantCosmicPresent =
			tealPixels.length > 0 || greenPixels.length > 0;
		expect(vibrantCosmicPresent).toBe(true);

		console.log(`  ✓ Vibrant Cosmic palette verified\n`);
	});

	test("should verify orbit scene is initialized @initialization", async ({
		page,
	}) => {
		...
		console.log(
			`  SceneInitializer: ${sceneStatus.sceneInitializerExists ? "✓" : "✗"}`,
		);
		console.log(
			`  WebGLMonitor: ${sceneStatus.webglMonitorExists ? "✓" : "✗"}`,
		);
		console.log(
			`  Orbit container: ${sceneStatus.orbitContainerExists ? "✓" : "✗"}`,
		);
		console.log(
			`  Orbit canvas: ${sceneStatus.orbitCanvasExists ? "✓" : "✗"} (${sceneStatus.canvasWidth}x${sceneStatus.canvasHeight})`,
		);
	...
	test("should verify CSS color variables are hex format @color-parsing", async ({
		page,
	}) => {
		...
		const colorVars = await page.evaluate(() => {
			...
			return {
				...
				cosmicPurpleBright: computedStyle
					.getPropertyValue("--cosmic-purple-bright")
					.trim(),
			};
		});
