...
test.describe("Constellation Mouse Interaction", () => {
	test.beforeEach(async ({ page }) => {
		...
		await page.waitForSelector("[data-constellation]", {
			state: "attached",
			timeout: 10000,
		});
		await page.waitForSelector(".constellation-canvas", {
			state: "attached",
			timeout: 5000,
		});
	...
	test("should have constellation canvas present @smoke", async ({ page }) => {
		...
		console.log(
			`  Container exists: ${canvasExists.containerExists ? "✓" : "✗"}`,
		);
		console.log(`  Canvas exists: ${canvasExists.canvasExists ? "✓" : "✗"}`);
		console.log(
			`  Canvas size: ${canvasExists.canvasWidth}x${canvasExists.canvasHeight}`,
		);
	...
	test("should verify pointer-events CSS allows mouse interaction @css-validation", async ({
		page,
	}) => {
		...
		console.log(
			`  Container pointer-events: ${pointerEventsConfig.containerPointerEvents}`,
		);
		console.log(
			`  Canvas pointer-events: ${pointerEventsConfig.canvasPointerEvents}`,
		);
	...
	test("should respond to mouse movement over canvas @interaction", async ({
		page,
	}) => {
		...
		console.log(
			`  Initial mouse position: (${initialState.mousePos.x}, ${initialState.mousePos.y})`,
		);
		...
		console.log(
			`  After move mouse position: (${afterMoveState.mousePos.x}, ${afterMoveState.mousePos.y})`,
		);
	...
	test("should influence particles when mouse is nearby @particle-behavior", async ({
		page,
	}) => {
		...
		console.log(
			`  Particles with velocity: ${particleState.particlesWithHighVelocity}`,
		);
		console.log(
			`  Mouse influence radius: ${particleState.mouseInfluenceRadius}px`,
		);
		console.log(
			`  Current mouse: (${particleState.mousePos.x.toFixed(0)}, ${particleState.mousePos.y.toFixed(0)})`,
		);
	...
	test("should track mouse position throughout canvas @mouseleave", async ({
		page,
	}) => {
		...
		console.log(
			`  Mouse during hover: (${duringHover.x.toFixed(0)}, ${duringHover.y.toFixed(0)})`,
		);
		...
		console.log(
			`  Mouse after corner move: (${afterMove.x.toFixed(0)}, ${afterMove.y.toFixed(0)})`,
		);
