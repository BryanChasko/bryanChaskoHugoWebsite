...
class SkillsNetworkScene extends BaseScene {
	...
	initializeNetwork() {
		...
		if (this.skillsData.length > 0) {
			const coreSkills = this.skillsData.slice(
				0,
				Math.min(5, this.skillsData.length),
			);
	...
	render(_deltaTime, _elapsed) {
		...
		for (const node of this.nodes) {
			...
			this.gl.uniform1f(
				this.uPointSize,
				node.radius * 2 + (node === this.selectedNode ? 4 : 0),
			);
	...
	setupFallback() {
		...
		for (const node of this.nodes) {
			...
			circle.setAttribute(
				"fill",
				`rgba(${Math.round(node.color[0] * 255)}, ${Math.round(node.color[1] * 255)}, ${Math.round(node.color[2] * 255)}, 0.8)`,
			);
