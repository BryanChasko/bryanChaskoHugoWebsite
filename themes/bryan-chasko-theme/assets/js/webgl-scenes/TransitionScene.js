...
class TransitionScene extends BaseScene {
	...
	createShaders() {
		...
		this.uNoiseTexture = this.gl.getUniformLocation(
			this.program,
			"noiseTexture",
		);
		this.uTransitionProgress = this.gl.getUniformLocation(
			this.program,
			"transitionProgress",
		);
	...
	update(_deltaTime, _elapsed) {
		...
		if (this.transitionActive) {
			const progress = Math.min(
				(Date.now() - this.transitionStartTime) / this.transitionDuration,
				1.0,
			);
