...
class WebGLSceneInitializer {
	...
	async init() {
		...
		if (
			...
			document.querySelector("[data-constellation]")
		) {
			this.initHomeScenes();
		}
	...
	initHomeScenes() {
		...
		if (
			orbitContainer &&
			!orbitContainer.querySelector(".webgl-scene-canvas")
		) {
			try {
				...
				console.log(
					`[OrbitScene] Responsive sizing: canvas=(${canvasWidth}x${canvasHeight}px), center=(${centerOffsetX.toFixed(1)}, ${centerOffsetY.toFixed(1)}), maxRadius=${maxRadius.toFixed(1)}px`,
				);
		...
		const shimmerContainer = document.querySelector(
			'[data-webgl-accent="shimmer"]',
		);
		if (
			shimmerContainer &&
			!shimmerContainer.querySelector(".webgl-scene-canvas")
		) {
			try {
	...
	initNoteScenes() {
		const articleCards = document.querySelectorAll(
			".builder-note-card, .article-card",
		);
		...
		articleCards.forEach((card, index) => {
			if (!card.querySelector(".webgl-scene-canvas")) {
				try {
					...
					console.warn(
						`Failed to initialize RippleScene on card ${index}:`,
						error,
					);
				}
			}
	...
	initSkillsScenes() {
		...
		if (
			!skillsContainer ||
			skillsContainer.querySelector(".webgl-scene-canvas")
		) {
			return;
		}
		...
		try {
			...
			if (skillsList) {
				skillsData = Array.from(skillsList.querySelectorAll("li")).map((li) =>
					li.textContent.trim(),
				);
			} else {
	...
	dispose() {
		...
		if (this.transitionScene?.destroy) {
