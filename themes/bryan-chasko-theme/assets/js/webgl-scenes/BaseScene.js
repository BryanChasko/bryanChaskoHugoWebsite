...
class BaseScene {
	...
	init() {
		try {
			...
			if (containerStyle.position === "static") {
				console.warn(
					`[${this.constructor.name}] Container lacks position context; canvas positioning may fail`,
				);
			}
	...
	createShaders() {
		...
		if (!this.gl.getProgramParameter(this.program, this.gl.LINK_STATUS)) {
			throw new Error(
				`Shader program failed to link: ${this.gl.getProgramInfoLog(this.program)}`,
			);
		}
	...
	startAnimating() {
		...
		console.log(
			"[BaseScene] startAnimating() called, loop starting, constructor:",
	...
	animate = () => {
		...
		if (!this.animateCalledOnce) {
			...
			console.log(
				"[BaseScene.animate] First frame, constructor:",
				this.constructor.name,
			);
		}
	...
	setupFallback() {
		...
		fallback.style.background =
			"radial-gradient(ellipse at center, rgba(129, 105, 197, 0.1) 0%, transparent 70%)";
