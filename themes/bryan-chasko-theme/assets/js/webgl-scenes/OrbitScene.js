...
class OrbitScene extends BaseScene {
	constructor(container, options = {}) {
		...
		if (rect.width === 0 || rect.height === 0) {
			console.warn(
				"[OrbitScene] Container has zero dimensions; particles may not render",
			);
		}
	...
	setupBuffers() {
		...
		this.positionAttrLocation = this.gl.getAttribLocation(
			this.program,
			"position",
		);
	...
	render(_deltaTime, _elapsed) {
		...
		if (window.orbitRenderCount % 120 === 1) {
			console.log(
				"[OrbitScene.render] Called, frame:",
				window.orbitRenderCount,
				"GL context:",
				!!this.gl,
			);
		...
		if (this.options.debug && !this.debugLogged) {
			...
			console.log("[OrbitScene] First render frame diagnostics:", {
				...
				programActive:
					this.gl.getParameter(this.gl.CURRENT_PROGRAM) === this.program,
		...
		if (!this.renderCallLogged) {
			...
			console.log(
				"[OrbitScene] render() called for first time, drawing particles...",
			);
		}
		...
		if (orbits.length === 0) {
			console.warn(
				"[OrbitScene] No orbit definitions available; skipping render",
			);
			return;
		}
		...
		const w =
			this.options.width || this.canvas.width || this.gl.drawingBufferWidth;
		const h =
			this.options.height || this.canvas.height || this.gl.drawingBufferHeight;

		// Debug viewport on first render
		if (!this.viewportDebugLogged) {
			this.viewportDebugLogged = true;
			console.log(
				`[OrbitScene] Viewport: canvas=${this.canvas.width}x${this.canvas.height}, drawingBuffer=${this.gl.drawingBufferWidth}x${this.gl.drawingBufferHeight}, using=${w}x${h}`,
			);
		}
		...
		for (let orbitIdx = 0; orbitIdx < orbits.length; orbitIdx++) {
			...
			for (let i = 0; i < orbit.particleCount; i++) {
				const angle =
					(i / orbit.particleCount) * Math.PI * 2 + time * orbit.speed;
			...
			if (this.options.debug && orbitIdx === 0 && !this.firstPositionLogged) {
				...
				console.log(
					`[OrbitScene] First particle: pos=(${firstPos_x.toFixed(1)}, ${firstPos_y.toFixed(1)}), center=(${this.centerX.toFixed(1)}, ${this.centerY.toFixed(1)}), dims=${w}x${h}, NDC=(${ndc_x.toFixed(2)}, ${ndc_y.toFixed(2)})`,
				);
			}
			...
			if (orbitIdx === 0 && !this.positionDataLogged) {
				...
				console.log(
					`[OrbitScene] Position buffer data (first 4 positions): [${positions
						.slice(0, 8)
						.map((p) => p.toFixed(0))
						.join(", ")}...]`,
				);
			...
			if (this.isHovering && this.hoverStartTime) {
				...
				// Speed boost
				const _speedBoost = 1 + pulseFactor * 0.5;
			...
			if (orbitIdx === 0 && !this.colorDebugLogged) {
				...
				console.log(
					`[OrbitScene] Teal color from CSS: [${color[0].toFixed(2)}, ${color[1].toFixed(2)}, ${color[2].toFixed(2)}]`,
				);
			}
			...
			if (
				this.options.debug &&
				orbitIdx === 0 &&
				window.orbitDrawCount === undefined
			) {
				window.orbitDrawCount = 0;
				const attrEnabled = this.gl.getVertexAttrib(
					this.positionAttrLocation,
					this.gl.VERTEX_ATTRIB_ARRAY_ENABLED,
				);
				console.log(
					`[OrbitScene] About to draw: attrEnabled=${attrEnabled}, positionAttrLoc=${this.positionAttrLocation}`,
				);
			}
			...
			if (this.options.debug && orbitIdx === 0 && window.orbitDrawCount === 0) {
				...
				console.log(
					"[OrbitScene] Drew orbit 0, particle count:",
					orbit.particleCount,
					"glError:",
					glError,
					"pixel at center:",
					Array.from(testPixel),
				);
			}
		}
		...
		this.gl.uniform1f(
			this.uPointSize,
			this.centerNode.radius + Math.sin(time * 2) * 3,
		);
