...
class WebGLResourceMonitor {
	...
	constructor(renderer, options = {}) {
		...
		this.scenes = []; // Track multiple scenes
		this.options = {
		...
		// Set as singleton
		if (!WebGLResourceMonitor.instance) {
			WebGLResourceMonitor.instance = this;
		}
	}

	/**
	 * Register a WebGL scene for monitoring
	 */
	static registerScene(scene) {
		if (!WebGLResourceMonitor.instance) {
			WebGLResourceMonitor.instance = new WebGLResourceMonitor();
			WebGLResourceMonitor.instance.start();
		}
		WebGLResourceMonitor.scenes.push(scene);
	}

	/**
	 * Unregister a WebGL scene
	 */
	static unregisterScene(scene) {
		const idx = WebGLResourceMonitor.scenes.indexOf(scene);
		if (idx > -1) {
			WebGLResourceMonitor.scenes.splice(idx, 1);
		}
	}
	 ...
	 * Get GPU tier level
	 */
	static get tierLevel() {
		if (!window.detector) {
			window.detector = new GPUCapabilityDetector();
		}
		return window.detector.detect().tier;
	}
	...
	resetIdleTimer() {
		...
		WebGLResourceMonitor.scenes.forEach((scene) => {
			if (scene?.isVisible && !scene.isAnimating && !scene.isDestroyed) {
	...
	startIdleTimer() {
		this.idleTimer = setTimeout(() => {
			...
			// Pause all scenes instead of disposing
			WebGLResourceMonitor.scenes.forEach((scene) => {
				if (scene?.stopAnimating) {
					scene.stopAnimating();
				}
			});
	...
	checkPerformance() {
		...
		if (performance.memory) {
			...
			if (memoryRatio > this.options.memoryThreshold) {
				console.warn(
					"[WebGL Monitor] High memory usage, triggering cleanup...",
				);
				this.callbacks.onLowPerformance.forEach((cb) =>
					cb("memory", memoryRatio),
				);
			}
		}
	}
...
class ThemeShaderHelper {
	...
	getThemeUniforms() {
		return {
			...
			uIsDarkMode:
				document.documentElement.dataset.theme === "dark" ? 1.0 : 0.0,
		};
	}
