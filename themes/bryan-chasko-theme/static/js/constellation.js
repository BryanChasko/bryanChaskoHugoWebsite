...
class ConstellationScene {
		...
		if (
			...
			if (!this._lastDebugLog || Date.now() - this._lastDebugLog > 1000) {
				console.log("[Constellation] Mouse over canvas:", {
					mousePos: {
						x: Math.round(this.mousePos.x),
						y: Math.round(this.mousePos.y),
					},
	...
	update(deltaTime) {
		...
		if (
			influencedCount > 0 &&
			(!this._lastInfluenceLog || Date.now() - this._lastInfluenceLog > 2000)
		) {
			console.log(
				"[Constellation] Particles influenced by mouse:",
				influencedCount,
				"/",
		...
		this.particles.forEach((p) => {
			...
			if (dist < this.options.mouseInfluence) {
				...
				const highlightAlpha =
					this.colors.particle[3] * (1 - t) + this.colors.highlight[3] * t;
				colors.push(
